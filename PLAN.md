...
- **Bump `fs4` 0.13.1 → 1.x** — `cargo install` flagged on v0.1.22 that fs4 has a newer major (`available: v1.1.0`). Used in `src/lock.rs` for the cross-platform `fs4::fs_std::FileExt` advisory-lock API; check whether the trait moved on the 1.x major before bumping. Low priority — current version works on every CI platform.
