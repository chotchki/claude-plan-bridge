...
Exercises every FORMATv2 feature the parser ships in Phase 36: v1 anchor
phases, v2 header phases, `*(depends on: ...)*` markers, `*(prefer after:
...)*` markers, phase-level prose (intro + between tasks + trailing), nested
subtasks, all four checkbox states, and h1 `# Backlog (not yet phased)` with
both flat-bullet notes and nested descoped subtrees.
